...
- 특징
  ...
  - 사용하지 말자!
    - 애플리케이션의 실제 코드와 관계 없는 <u>테스트 코드</u>
    - 스프링 설정을 목적으로 하는 @Configuraion 같은 곳에서만 특별한 용도로 사용
...
- `Optional<> : ` 자동 자입할 대상이 없으면 `Optional.empty` 가 입력된다.
...
- 생성자 주입 방식을 선택하는 여러가지 이유가 있지만, 프레임워크에 의존하지 않고, 순수한 자바 언어의 특징을 잘 살리는 방법이기도 하다.
...
이 최종결과 코드와 이전의 코드는 완전히 동일하다. 롬복이 자바의 애노테이션 프로세서라는 기능을 이용해서 컴파일 시점에 생성자 코드를 자동으로 생성해준다. 실제 `class`를 열어보면 다음 코드가 추가되어 있다는 것을 확인할 수 있다.

**정리**
최근 생성자를 딱 1개 두고, `@Autowired`를 생략하는 방법을 주로 사용한다. 여기에 Lombok 라이브러리의 `@RequiredArgsConstructor`함께 사용하면 기능은 다 제공하면서, 코드는 깔끔하게 사용할 수 있다.
...
이때 하위 타입으로 지정할 수 있지만, 하위 타입으로 지정하는 것은 DIP를 위배하고 유연성이 떨어진다. 그리고 이름만 다르고, 완전히 똑같은 타입의 스프링 빈이 2개 있을 때 해결이 안된다.
...
`@Qualifier`의 단점은 주입 받을 때 다음과 같은 모든 코드에 `@Qualifier`를 붙여주어야 한다는 점이다.
...
`@Qualifier("mainDiscountPolicy")` 이렇게 문자를 적으면 컴파일시 타입 체크가 안된다. 다음과 같은 애노테이션을 만들어 문제를 해결할 수 있다.
...
예를 들어서 할인 서비스를 제공하는데, 클라이언트가 할인의 종류(rate, fix)를 선택할 수 있다고 가정해보자. 스프링을 사용하면 소위 말하는 전략 패턴을 매우 간단하게 구현 할 수 있다.

###### 로직 분석

- DiscountService는 Map으로 모든 `DiscountPolicy`를 주입 받는다. 이떄 `fixDiscountPolicy, rateDiscountPolicy`가 주입된다.
...
- 만약 해당하는 타입의 스프링 빈이 없다면, 빈 컬랙션이나 Map을 주입한다.
...
다형성을 적극 활용하는 비지니스 로직은 수동 등록을 구민해보자
